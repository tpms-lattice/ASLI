		...
		for (int i = 1; i < argc; i++) {
			...
			if( strcmp(argv[i], "-help")==0 || strcmp(argv[i], "--help")==0 ) {
				...
				std::cout
					...
					<< "  \n"
					<< "  -exportls              : Generate level-set sol file and corresponding base mesh without discretizing the level-set (MMG option)\n"
				<< std::endl;
...
if (configFile.has_filename() == false)
	...
	ASLI *data = new ASLI(configFile.u8string(), exportls);
