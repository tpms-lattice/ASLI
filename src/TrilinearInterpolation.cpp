...
bool TrilinearInterpolation::setup(std::vector<std::vector<double>> data, 
...
bool TrilinearInterpolation::setup(Eigen::Matrix3Xd coordinates, Eigen::VectorXd y, 
                                   modelData *interpolationModel) {
...
//	}
	...
	Eigen::MatrixXd interpolationMatrix_augmented(interpolationMatrix.rows()+coordinates.rows()+1, interpolationMatrix.cols()+coordinates.rows()+1);

	interpolationMatrix_augmented << interpolationMatrix, P, 
	                                P.transpose(), zerosMat;
	...
	// Compute the interpolation coefficents
	interpolationModel->interpolationCoefficients = interpolationMatrix_augmented.partialPivLu().solve(y_augmented); // Using LU decomposition with partial pivoting (considering distance matrix will always be invertible)

	return EXIT_SUCCESS;
}

double TrilinearInterpolation::evaluate(Eigen::Vector3d p, 
                                        modelData *interpolationModel) {
	/* Evaluates the RBF-interpolation model
	 * Inputs:
	 *   point              : Point for which output is to be estimated
	 *   interpolationModel : Structure containing the interpolation model data
	 * Return:
...
Eigen::MatrixXd TrilinearInterpolation::internal::distanceMatrix(Eigen::Matrix3Xd *A, Eigen::Matrix3Xd *B) {
...
Eigen::VectorXd TrilinearInterpolation::internal::distanceVector(Eigen::Matrix3Xd *A, Eigen::Vector3d *B) {
...
double TrilinearInterpolation::internal::kernel(double r) {
	/* Basis function */
